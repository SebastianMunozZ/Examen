...
from .serializers import Formulario_serializer, Productoserializer, Tipo_mascota_serializer, Donacion_serializer,Venta,Detalle_venta
...
@csrf_exempt
@api_view(['GET', 'POST'])
def registrarventa(request,id):
    if request.method == 'GET':
        dona = Donacion.objects.get(id_don = id)
        dona.delete()
        return Response('Eliminado', status=status.HTTP_201_CREATED)
    elif request.method == 'POST':
        venta = JSONParser().parse(request)
        #Lógica para registrar venta en las correspondientes tablas
        dona = Donacion.objects.get(id_don = id)
        serializer = Donacion_serializer(dona,data=data)
        if serializer.is_valid():
            serializer.save()
